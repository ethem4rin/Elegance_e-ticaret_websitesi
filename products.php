...
$defaultCategories = ['Erkek', 'Kadın', 'Aksesuarlar', 'Çanta', 'Çocuk', 'Ayakkabı'];
$existingCategories = $pdo->query('SELECT category_name FROM categories')->fetchAll(PDO::FETCH_COLUMN);
$missingCategories = array_diff($defaultCategories, $existingCategories);
...
$catStmt = $pdo->prepare('SELECT * FROM categories WHERE category_name <> ? ORDER BY category_name');
$catStmt->execute(['Unisex']);
...
<div class="page-banner">
    <div class="container">
        <h1 class="page-banner__title">Mağaza</h1>
        <p class="page-banner__subtitle">Tüm koleksiyonumuzu keşfedin</p>
    </div>
</div>
...
<div class="container shop-layout">
    ...
    <aside class="shop-sidebar">
        <form method="GET" action="" id="filter-form">
            <div class="sidebar__section">
                <h3 class="sidebar__title">Arama</h3>
                <div class="sidebar__search">
                    <input type="text" name="search" id="search-input" value="<?= htmlspecialchars($search) ?>" placeholder="Ürün ara…" class="form-control">
                </div>
            </div>
            ...
            <button type="submit" class="btn btn--primary btn--full">Filtreleri Uygula</button>
            <?php if ($search || !empty($categoryIds)): ?>
                <a href="<?= SITE_URL ?>/products.php" class="btn btn--secondary btn--full" style="margin-top:8px;">Filtreleri Temizle</a>
            <?php endif; ?>
        </form>
    </aside>
    ...
    <div class="shop-main">
        <div class="shop-header">
            <p class="shop-count">
                <?php if ($search): ?>
                    "<?= htmlspecialchars($search) ?>" için <?= $totalProducts ?> sonuç
                <?php else: ?>
                    <?= $totalProducts ?> ürün
                <?php endif; ?>
            </p>
        ...
        <div class="empty-state">
            ...
            <p>Ürün bulunamadı.</p>
            <a href="<?= SITE_URL ?>/products.php" class="btn btn--secondary">Tümünü Gör</a>
        </div>
        <?php else: ?>
        ...
        <!-- Pagination -->
        <?php if ($totalPages > 1): ?>
        <nav class="pagination" aria-label="Ürün sayfalaması">
            <?php
            ...
            ?>
            <?php if ($page > 1): ?>
                <a href="?<?= $queryBase ?>&page=<?= $page - 1 ?>" class="pagination__btn">← Önceki</a>
            <?php endif; ?>
            ...
            <?php if ($page < $totalPages): ?>
                <a href="?<?= $queryBase ?>&page=<?= $page + 1 ?>" class="pagination__btn">Sonraki →</a>
            <?php endif; ?>
        </nav>
        <?php endif; ?>
        <?php endif; ?>
