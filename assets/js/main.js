...
window.addToCart = async function(productId, quantity = 1) {
  try {
    ...
    if (data.code === 'NOT_AUTHENTICATED') {
      showToast('Sepetinize ürün eklemek için giriş yapın.', 'error');
    ...
    if (data.success) {
      ...
      showToast(data.error || 'Sepete eklenemedi', 'error');
    }
  } catch {
...
async function removeCartItem(cartId, row) {
  ...
  } catch { showToast('Ürün kaldırılırken hata oluştu', 'error'); }
}
...
async function updateCartItem(cartId, qty, row, input) {
  ...
  cartUpdateTimers[cartId] = setTimeout(async () => {
    ...
    } catch { showToast('Sepet güncellenirken hata oluştu', 'error'); }
  }, 350);
}
...
function recalcCartTotals() {
  ...
  if (shippingEl) shippingEl.textContent = shipping === 0 ? 'Ücretsiz' : '₺9.99';
