...
if ($method === 'POST') {
    if (!isLoggedIn()) {
        ...
        echo json_encode(['error' => 'Giriş yapılmadı', 'code' => 'NOT_AUTHENTICATED']);
        exit;
    }
    ...
    if ($action === 'add') {
        ...
        if ($productId <= 0) {
            http_response_code(400);
            echo json_encode(['error' => 'Geçersiz ürün kimliği']);
            exit;
        }
        ...
        if (!$row) {
            http_response_code(404);
        echo json_encode(['error' => 'Sepet ürünü bulunamadı']);
            exit;
        }
...
echo json_encode(['error' => 'İzin verilmeyen yöntem']);
